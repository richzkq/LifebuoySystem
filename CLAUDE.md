...
Smart Lifebuoy System (智能救生圈系统) — an IoT drowning detection and rescue monitoring platform. An RK3588 edge device runs AI models (RKNN) to detect drowning persons in real-time video, pushes detection results and video frames to a Spring Boot server on Alibaba Cloud (47.83.199.93), which streams them to a Vue 3 web dashboard and a uni-app mobile client.
...
cd frontend
npm install
npm run dev       # Dev server at localhost:5173, proxies to 47.83.199.93:8080
...
| `frontend/.env.development` | `VITE_API_BASE_URL=http://47.83.199.93:8080` |
