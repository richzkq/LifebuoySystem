...
Smart Lifebuoy System (智能救生圈系统) — an IoT drowning detection and rescue monitoring platform. An RK3588 edge device runs AI models (RKNN) to detect drowning persons in real-time video, pushes detection results and video frames to a Spring Boot server on Alibaba Cloud (120.55.101.95), which streams them to a Vue 3 web dashboard and a uni-app mobile client.
...
cd frontend
npm install
npm run dev       # Dev server at localhost:5173, proxies to 120.55.101.95:8080
...
| `frontend/.env.development` | `VITE_API_BASE_URL=http://120.55.101.95:8080` |
